...
ORDER BY t.dataTorneio DESC, numeroProva ASC;";
    ...
    {
        $prova = "INSERT INTO tb_prova (numeroProva, genero, ID_TORNEIO, ID_DISTANCIAESTILO, ID_CATEGORIA_MIN, ID_CATEGORIA_MAX) VALUES (:numeroProva, :genero, :id_torneio, :id_distanciaestilo, :id_categoria_min, :id_categoria_max)";
        $stmt = $this->db->prepare($prova);
    ...
    {
        $prova = "UPDATE tb_prova SET numeroProva = :numeroProva, genero = :genero, ID_TORNEIO = :id_torneio, ID_DISTANCIAESTILO = :id_distanciaestilo, ID_CATEGORIA_MIN = :id_categoria_min, ID_CATEGORIA_MAX = :id_categoria_max WHERE IDPROVA = :idprova";
        $stmt = $this->db->prepare($prova);
    ...
    public function deleteProva()
    {
        $prova = "DELETE FROM tb_prova WHERE IDPROVA = :idprova";
        $stmt = $this->db->prepare($prova);
        $stmt->bindValue(':idprova', $this->__get('idprova'));
        $stmt->execute();
        return $this;
    ...
    {
        $prova = "SELECT idprova ,numeroProva, genero, id_torneio, ID_CATEGORIA_MIN, ID_CATEGORIA_MAX, id_distanciaestilo, t.nomeTorneio, t.dataTorneio, f.nomeFantasiaFederacao, p.tamanhoPiscina, d.distancia, e.nomeEstilo, cmin.nomeCategoria AS categoriaMinima, cmax.nomeCategoria AS categoriaMaxima FROM tb_prova
        INNER JOIN tb_torneio AS t ON ID_TORNEIO = t.IDTORNEIO
        INNER JOIN tb_federacao AS f ON 
        (SELECT tb_torneio.ID_FEDERACAO FROM tb_torneio WHERE tb_prova.ID_TORNEIO = tb_torneio.IDTORNEIO) = f.IDFEDERACAO
        INNER JOIN tb_distancia AS d ON 
